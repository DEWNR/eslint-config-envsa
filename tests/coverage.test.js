...
describe('Do we have tests for all the rules that we specify', () => {
  test('one test file per eslint base rule', () => {
    ...
    const testNames = new Set(tests.map((filename) => path.basename(filename, '.test.js')));

    expect(ruleNames.every((rule) => testNames.has(rule))).toBe(true);
  });
  ...
  test('one test file per eslint import rule', () => {
    ...
    const testNames = new Set(
      tests.map((filename) => `import-x/${path.basename(filename, '.test.js')}`)
    );
  ...
  test('one test file per eslint unicorn rule', () => {
    ...
    const testNames = new Set(
      tests.map((filename) => `unicorn/${path.basename(filename, '.test.js')}`)
    );
  ...
  test('one test file per stylelint scss rule', () => {
    ...
    const testNames = new Set(
      tests.map((filename) => `scss/${path.basename(filename, '.test.js')}`)
    );
